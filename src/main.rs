...
async fn main() -> Result<(), JobSchedulerError> {
    ...
    std::env::var("LOG_FILE").unwrap().to_string();
    let args = [
        "-av".to_string(),
            ...
            .add(Job::new("1 12 * * * *", move |_, _| {
                ...
                let success = execute(
                    ...
                    [
                        ...
                        format!(
                            "--compare-dest='{}'",
                            format!("{}@{}:{}/hourly/{}", user, server, target_folder, last_hour)
                        ),
            ...
            .add(Job::new("1 14 1 * * 1", move |_, _| {
                ...
                let success = execute(
                    ...
                    [
                        ...
                        format!(
                            "--compare-dest='{}'",
                            format!("{}@{}:{}/weekly/{}", user, server, target_folder, last_week)
                        ),
                        format!("{}@{}:{}/weekly/{}", user, server, target_folder, week),
                    ]
                    .to_vec(),
                ...
                let success = execute(
                    ...
                    [
                        ...
                        format!(
                            ...
                            format!(
                                "{}@{}:{}/monthly/{}",
                                user, server, target_folder, last_month
                            )
                        ),
                        format!("{}@{}:{}/monthly/{}", user, server, target_folder, month),
                    ]
